    ...
    if (starting_) {
      ...
      if (updateCount % 20 == 0) {
        ...
        if (keyboard && keyboard->isKeyboardAvailable() && mouse && mouse->isMouseAvailable())
          ...
          canvas.drawText(70, 75, "Press a [BUTTON] to Play");
      }

      // handle keyboard or mouse (after two seconds)
      if (updateCount > 50) {
        if (keyboard && keyboard->isKeyboardAvailable() && keyboard->isVKDown(fabgl::VK_SPACE) || digitalRead(button4) == LOW)
        {
...
struct GameScene : public Scene {
  ...
  void gameOver()
  {
    ...
    if (IntroScene::controller_ == 1)
      canvas.drawText(110, 100, "Press [BUTTON]");
  ...
  void update(int updateCount)
  {
    ...
    if (gameState_ == GAMESTATE_PLAYING || gameState_ == GAMESTATE_PLAYERKILLED) {
      ...
      if (IntroScene::controller_ == 1) {
        // KEYBOARD controller
        if (keyboard->isVKDown(fabgl::VK_LEFT) || mov == 3)
          playerVelX_ = -1;
        else if (keyboard->isVKDown(fabgl::VK_RIGHT) || mov == 4)
          playerVelX_ = +1;
        else
          playerVelX_ = 0;
        if ((keyboard->isVKDown(fabgl::VK_SPACE) || digitalRead(button4) == LOW) && !playerFire_->visible)  // player fire?
          fire();
      } else if (IntroScene::controller_ == 2) {
        // MOUSE controller
